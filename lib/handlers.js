...
// Users - delete
/**Required data: phone
* @TODO only allow an authenticated user delete only their own object
            ...
            if (!err && data) {
                ...
                _data.delete('users', phone, (err) => {
                    if (!err) {
                        ...
                        callback(500, {Error: 'Could not delete the psecified user.'});
                    }
                });
            }else {
                callback(404, {Error: 'User not found.'})
...
handlers._tokens.POST = (data, callback) => {
  ...
  if (phone && password) {
    // Lookup user whi matches that phone number
    _data.read('users', phone, (err, userData) => {
        if (!err && userData) {
...
handlers._tokens_PUT = (data, callback) => {
